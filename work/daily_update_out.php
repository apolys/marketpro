...
//textvalue	
 ...
 $issue = ($_POST['issue']);
 $marketinfo = ($_POST['marketinfo']);

 $q_no = $modi_no;

 $q_no = ($_POST['q_no']);//체크아웃을 위해 추가 적용


echo "$purpose;$store;$time_in;$time_in2;$modi_no;$gps_lat;$gps_lng;$q_no;";
//exit;
//입력데이터 로드
...
$c_no = 1;
	...
	$result = mysqli_query($connection, "SELECT * FROM daily where  no='$q_no' order by no ");
	while($rows = mysqli_fetch_array($result)) {
		$cn_lat = $rows['lat'];
		$cn_lng = $rows['lng'];
	}
$old_gps = ($cn_lat+$cn_lng);
$new_gps = ($gps_lat+$gps_lng);
$gps_calculation = ($cn_lat+$cn_lng) - ($gps_lat+$gps_lng);

echo "<br>$old_gps;$new_gps;<br>";

if($q_no){
//update query
   $query = "update daily set  location='$gps_calculation' , marketinfo='$marketinfo' where no='$q_no' ";
